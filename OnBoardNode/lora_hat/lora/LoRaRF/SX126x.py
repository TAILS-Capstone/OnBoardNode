...
import spidev
from gpiozero import Device, Button as Event

import time

spi = spidev.SpiDev()

# For General Purpose GPIOs
gpio_pins = Device.pin_factory
gpio_low = False
gpio_high = True
...
class SX126x(BaseLoRa):
    ...
    def reset(self) -> bool:
        ...
        gpio_pins.pin(self._reset).function = "output"
        gpio_pins.pin(self._reset).state = gpio_low
        time.sleep(0.001)
        gpio_pins.pin(self._reset).state = gpio_high
    ...
    def wake(self):
        ...
        if self._wake != -1:
            gpio_pins.pin(self._wake).function = "output"
            gpio_pins.pin(self._wake).state = gpio_low
            time.sleep(0.0005)
        self.setStandby(self.STANDBY_RC)
        self._fixResistanceAntenna()
    ...
    def busyCheck(self, timeout: int = _busyTimeout):
        ...
        t = time.time()
        gpio_pins.pin(self._busy).function = "input"  # Set the busy pin as input
        while gpio_pins.pin(self._busy).state == gpio_high:
    ...
    def setPins(
        ...
        gpio_pins.pin(reset).function = "output"
        gpio_pins.pin(busy).function = "input"
        gpio_pins.pin(self._cs_define).function = "output"
        if irq != -1:
            gpio_pins.pin(irq).function = "input"
        if txen != -1:
            gpio_pins.pin(txen).function = "output"
        # if rxen != -1 : gpio_pins.pin(rxen).function = "output"
    ...
    def beginPacket(self):
        ...
        if self._txen != -1:
            self._txState = gpio_pins.pin(self._txen).state  # Read the current state of the txen pin
            gpio_pins.pin(self._txen).state = gpio_low          # Set the txen pin to LOW
    ...
    def endPacket(self, timeout: int = TX_SINGLE) -> bool:
        ...
        if self._irq != -1 :
            irq_pin = Event(self._irq, pull_up=True)  # Assumes the IRQ pin is normally high
            irq_pin.when_pressed = None # Remove the previous event 
            irq_pin.when_pressed = self._interruptRx  # Triggers on rising edge
        return True
    ...
    def request(self, timeout: int = RX_SINGLE) -> bool:
        ...
        if self._txen != -1:
            self._txState = gpio_pins.pin(self._txen).state  # Read the current state of the TX enable pin
            gpio_pins.pin(self._txen).state = gpio_high         # Set the TX enable pin to HIGH
        ...
        if self._irq != -1 :
            ...
            irq_pin.when_pressed = None # Remove the previous event 
            if timeout == self.RX_CONTINUOUS :
                irq_pin.when_pressed = self._interruptRxContinuous  
            else :
                irq_pin.when_pressed = self._interruptRx
        return True
    ...
    def wait(self, timeout: int = 0) -> bool:
        ...
        if self._statusIrq:
            ...
            if self._txen != -1:
                gpio_pins.pin(self._txen).state = self._txState
    ...
    def _writeBytes(self, opCode: int, data: tuple, nBytes: int):
        if self.busyCheck():
            return
        gpio_pins.pin(self._cs_define, gpio_low)
        buf = [opCode]
        for i in range(nBytes):
            buf.append(data[i])
        spi.xfer2(buf)
        gpio_pins.pin(self._cs_define, gpio_high)

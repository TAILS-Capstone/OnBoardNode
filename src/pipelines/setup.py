...
def check_hailo_package():
    ...
    try:
        ...
        logger.error(
            "Hailo python package not found. Please make sure you're in the Hailo virtual environment. Run 'source setup_env.sh' and try again."
        )
        sys.exit(1)
...
def main():
    ...
    setup(
        ...
        package_data={
            "hailo_apps_infra": ["*.json", "*.sh", "*.cpp", "*.hpp", "*.pc"]
            + get_downloaded_files(),
        },
        include_package_data=True,
        entry_points={
            "console_scripts": ["get-usb-camera=hailo_apps_infra.get_usb_camera:main"],
        },
    )
